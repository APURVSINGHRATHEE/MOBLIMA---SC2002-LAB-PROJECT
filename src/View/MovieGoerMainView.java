...
import Controller.*;
import View.StaffKickOffView;

import Model.Movie.MovieMain;
import View.MovieGoerBookingView;
...
public class MovieGoerMainView{
     ...
     */
	public static Hashtable<MovieMain,ArrayList<KickOff>> KickOffList;
    
    /**
     * Get the staff show time instance
     * @return the staff show time instance
     */
    private static MovieGoerMainView instance = null;
     ...
     */
	public void mainView(MovieGoerMenu mg) {
		moviegoer = mg;
		mainView();
	}
     ...
     */
    public static  MovieGoerMainView getInstance() {
        if(instance==null) {
            instance = new MovieGoerMainView();
        }
        return instance;
    }
    ...
    public static void listAllMovieDetailsView() {
        ...
        ArrayList<MovieMain> movies = MovieController.getInstance().getMovieList();
        for(int i=0; i < vMovieList.size(); i++) {
        	if(movies.get(i).showingStatus == movies.get(i).showingStatus.ENDOFSHOWING) {
        		continue;
        	}
        	else {
    ...
    public static void searchMovieDetailsView() {
        ...
        boolean found = false;
        Scanner sc = new Scanner(System.in);
        ArrayList<MovieMain> vMovieList = MovieController.getInstance()
                .getMovieList();
        outerloop:
        do {
    ...
    public void getIndividualMovieDetailsView(String movieTitle){
        ...
        while (true) {
            ...
            for (int i=0; i<vMovieList.size(); i++){
                if (vMovieList.get(i).getMovieTitle().equals(movieTitle)){
                    System.out.println("Movie Title: " + vMovieList.get(i)
                            .getMovieTitle());
                    if (vMovieList.get(i).getAvgremark() == -1) {
                    ...
                    System.out.println("Movie Genres: " 
                            + vMovieList.get(i).getGenre());
                    System.out.println("Movie Rated: " 
                            + vMovieList.get(i).getremark());
            ...
            switch(choice) {
                case 1:
                    MovieGoerBookingView.getInstance().mainView(MovieController.getInstance().getMovie(movieTitle));
                    break;
                case 2:
    ...
    public void seeAllMovieRatingView(String movieTitle){
        ...
        ArrayList<Remarks> reviewList = MovieController.getInstance()
        ...
        if(reviewList == null) {
            ...
            for (int i = 0; i < reviewList.size(); i++) {
                ...
                System.out.println("Reviewer: " 
                    + reviewList.get(i).getremarkerName());
                System.out.println("Rating: " 
                    + reviewList.get(i).getremarkRating());
                System.out.println("Review Description: " 
                    + reviewList.get(i).getremark());
            }
        }
        do{
            System.out.println(" ");
            ...
            if(MovieController.getInstance().getTopFiveMovieRating()==null) {
                ...
                for(int i=0; i<5;i++) {
                    if (topFiveMovie[i] == null) {
                            ...
                            System.out.println("("+ (i+1) + ") "
                                    + topFiveMovie[i].getMovieTitle() 
                                    + " Ratings: "+ topFiveMovie[i].getAvgremark());	
                    }    
                }
                System.out.println("(0) Back");
            }
